...
# 32公里以上部分,每增加1元可乘坐20公里
     ...
     # 分析:假设50 (32+18) -> 6+1
     # 分析:假设52(32+20) -> 6+1
     # 分析:假设53(32+20+1) -> 6+1+1
     # 定义一个变量 求公里数的差值
     temp_km =km - 32
     # 分两种情况 tmep_km 是否可以被20整除
     # 整除了
     if temp_km % 20 == 0:
         # 32公里以内的 加上超出的
...
# 满150元以后的乘次,价格给予5折优惠
# 支出累计达400元以后的乘次,不再享受打折优惠

# 假设每个月,小明需要上20天班,每次上班需要来回1次,即每天需要乘坐两次同样路线的地铁
# 最终得出这"20天"小明坐地铁消费多少钱? (20天,乘坐40次)
# 定义一个变量 保存总的消费金额
total_money = 0
# 循环 模拟40次
for i in range(40):
